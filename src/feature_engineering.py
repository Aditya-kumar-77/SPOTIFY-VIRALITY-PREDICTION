...
class FeatureEngineer:
    ...
    # ------------------------------------------------------------------ #
    #  Save/Load artifacts for inference
    # ------------------------------------------------------------------ #
    def save_artifacts(self, path):
        """Save encoder and artist stats for inference"""
        import joblib
        os.makedirs(path, exist_ok=True)
        artifacts = {
            'target_encoder': self.target_encoder,
        }
        joblib.dump(artifacts, os.path.join(path, 'feature_engineer_artifacts.joblib'))
...
def main():
    ...
    parser.add_argument('--save-artifacts', type=str, default=None,
                        help='Directory to save feature engineer artifacts for inference')
    ...
    if args.save_artifacts:
        os.makedirs(args.save_artifacts, exist_ok=True)
        engineer.save_artifacts(args.save_artifacts)
        artist_stats = df_transformed[['artist_name', 'artist_avg_popularity', 'artist_song_count', 
                                       'artist_max_popularity', 'artist_std_popularity', 'artist_has_viral_hit']].drop_duplicates('artist_name') if 'artist_name' in df_transformed.columns else pd.DataFrame()
        artist_stats.to_csv(os.path.join(args.save_artifacts, 'artist_stats.csv'), index=False)
        print(f"\nArtifacts saved to {args.save_artifacts}")
