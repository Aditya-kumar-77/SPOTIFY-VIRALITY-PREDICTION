...
class SpotifyDataCollector:
    ...
    def _generate_sample_data(self, num_songs):
        ...
        Uses a mixture distribution to ensure ~20-25% of songs are 'viral' (popularity > 70),
        which is necessary for stratified train/test splitting in the ML pipeline.
        """
        np.random.seed(42)
        ...
        # Ensure a minimum number of viral songs for ML training
        # ~20% viral (popularity > 70), ~80% non-viral
        num_viral = max(int(num_songs * 0.20), 5)  # at least 5 viral songs
        num_non_viral = num_songs - num_viral
        
        # Generate non-viral songs (popularity 0-70)
        non_viral_pops = np.clip(np.random.beta(2, 5, size=num_non_viral) * 85, 0, 69).astype(int)
        # Generate viral songs (popularity 71-100)
        viral_pops = np.clip(71 + np.random.beta(3, 2, size=num_viral) * 29, 71, 100).astype(int)
        
        all_pops = np.concatenate([non_viral_pops, viral_pops])
        np.random.shuffle(all_pops)
